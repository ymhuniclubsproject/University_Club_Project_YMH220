...
def home(request):
    ...
    # 1. Top Rated Events
    top_rated_events = Event.objects.filter(is_approved=True).annotate(avg_score=Avg('ratings__score')).filter(avg_score__isnull=False).order_by('-avg_score')[:3]
    
    # 2. Most Popular Events (by registrations)
    most_popular_events = Event.objects.filter(is_approved=True).annotate(reg_count=Count('registrations')).filter(reg_count__gt=0).order_by('-reg_count')[:3]
    
    # 3. Top Rated Clubs (by average of their events' ratings)
    top_rated_clubs = Club.objects.filter(is_active=True, is_approved=True).annotate(avg_score=Avg('events__ratings__score')).filter(avg_score__isnull=False).order_by('-avg_score')[:3]
    
    # 4. Most Popular Clubs (by approved members)
    most_popular_clubs = Club.objects.filter(is_active=True, is_approved=True).annotate(member_count=Count('membership_requests', filter=Q(membership_requests__status='approved'))).filter(member_count__gt=0).order_by('-member_count')[:3]
    ...
    return render(request, 'app/index.html', {
        ...
        'most_popular_clubs': most_popular_clubs,
    })
...
def api_chat(request):
    if request.method == 'POST':
        try:
            ...
            club_info = "Clubs:\n" + "\n".join(
                [f"- {c.name}: {c.description[:100]}..." for c in clubs]
            )
            event_info = "Upcoming Events:\n" + "\n".join(
                [f"- '{e.title}' is organized by {e.club.name} on {e.event_date}." for e in events]
            )

            system_prompt = f"""You are a friendly, enthusiastic AI assistant for the UNICLUBS University Club & Event Management Ecosystem at Fırat University. Your task is to help users with information about clubs and events.
Here is the current club and event data:
...
3. If asked about something you don't know, say "I'm sorry, I don't have information about that."
"""
