...
class MessageRepository: # Manages messaging logic and conversation history between clubs | Kulüpler arası mesajlaşma mantığını ve konuşma geçmişini yönetir
    ...
    @staticmethod
    def get_conversation(db: Session, club_a: int, club_b: int): # Fetches chronological chat history between two specific clubs | İki belirli kulüp arasındaki kronolojik sohbet geçmişini getirir
        return db.query(Message).filter(
            or_(
